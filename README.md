...
- **A pull request cannot introduce a vulnerability.** [OSV-Scanner](https://google.github.io/osv-scanner/) scans the base branch and this one, and fails on the advisories only this branch adds, so a vulnerability somebody else introduced never blocks you. It reads `package-lock.json` and `bun.lock`, so both kinds of template are covered. `make audit_lockfiles` runs the same scanner locally, where it reports everything it finds rather than only what you added.
- **npm and bun are both first-class.** `make lockfiles` writes each template's lockfile with the package manager its runtime declares. Renovate keeps a separate `bun` manager, so `renovate.json5` enables both.
- **The lockfile has to match the runtime the template declares.** The Pulumi CLI picks a project's package manager by looking for these files, so a stray `yarn.lock` or `pnpm-lock.yaml` would hand the template to a package manager it never declared. A `nodejs` template gets npm's lockfile; a `bun` template always installs with bun, so it gets bun's. `make test_lockfiles` reads each `Pulumi.yaml` and fails on a project whose lockfile disagrees with its runtime, on a lockfile with no `package.json` beside it, and on a lockfile belonging to any other package manager.
- **Lockfiles must stay out of `.gitignore`.** Publishing a template archives its directory with git-ignore rules applied, so an ignored lockfile stays in the repo but never reaches the users who need it. `make test_lockfiles` publishes every template against a fake backend and fails if a lockfile is missing from the archive.
